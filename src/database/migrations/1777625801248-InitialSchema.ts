...
export class InitialSchema1777625801248 implements MigrationInterface {
    name = 'InitialSchema1777625801248'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TYPE "public"."tasks_status_enum" AS ENUM('active', 'completed')`);
        await queryRunner.query(`CREATE TABLE "tasks" ("id" SERIAL NOT NULL, "title" character varying(255) NOT NULL, "description" text, "estimated_hours" numeric(10,2) NOT NULL, "due_date" date NOT NULL, "status" "public"."tasks_status_enum" NOT NULL DEFAULT 'active', "cost" numeric(10,2) NOT NULL, "actual_hours" numeric(10,2), "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_8d12ff38fcc62aaba2cab748772" PRIMARY KEY ("id"))`);
        ...
        await queryRunner.query(`CREATE TABLE "user_tasks" ("task_id" integer NOT NULL, "user_id" integer NOT NULL, CONSTRAINT "PK_a22b2734df8bba2b8b7ebd3ae28" PRIMARY KEY ("task_id", "user_id"))`);
        ...
        await queryRunner.query(`ALTER TABLE "user_tasks" ADD CONSTRAINT "FK_67a8a20c2e44bfb84ca1a33e6df" FOREIGN KEY ("task_id") REFERENCES "tasks"("id") ON DELETE CASCADE ON UPDATE CASCADE`);
